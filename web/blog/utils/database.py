...
class BaseConx:
    def __init__(self, dsn, query_string, dbopt, format_string=None):
        self.dsn = dsn
        self.query_string: str = self.format_query_string(query_string, format_string)
        self.dbopt = dbopt

    def format_query_string(self, query_string, format_string):
        if format_string:
            query_string = self._format_query_string(query_string, format_string)
...
class Sqlite3Conx(BaseConx):
    ...
    def establish_conx(self):
        if not os.path.isabs(self.dsn):
            raise Exception(f"invalid absolute path {self.dsn}")
...
class PostgresConx(BaseConx):
    ...
    def establish_conx(self):
        return p.connect(self.dsn, cursor_factory=RealDictCursor)
...
def push(abspath, dsn):
    ...
    if ret:
        format_string = ""
        for i in ret:
            format_string += "{item},".format(item=i)
        p = PostgresConx(
            dsn,
            """
            INSERT INTO history (
        ...
        _, error = p.run()
        if not error:
            page_titles = tuple([x[0] for x in ret])
            s = Sqlite3Conx(
                abspath,
                """
                UPDATE history
